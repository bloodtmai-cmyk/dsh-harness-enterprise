...
// locations reaching the prose as an opener. A preload-shaped mock bridge lets
// this scenario drive the click without launching an application on the test
// machine and verifies the desktop-only right-side preview assembly.
...
describe('web e2e: inline-code mentions of produced files', () => {
  ...
  beforeAll(async () => {
    ...
    await page.addInitScript(() => {
      Object.defineProperty(window, 'desktopPreview', {
        configurable: true,
        value: {
          readFile: async () => ({
            ok: true,
            file: {
              name: 'report.html',
              displayPath: 'site/report.html',
              mime: 'text/html',
              kind: 'html',
              content: '<!doctype html><html><body><h1>Preview ready</h1><p>Rendered inside Harness Enterprise Desktop.</p></body></html>',
            },
          }),
  ...
  it.skipIf(MODE === 'record')('links the unique mention and opens it in the desktop preview panel', async () => {
    ...
    await expect.poll(() => page.getByRole('tab', { name: 'Preview' }).getAttribute('aria-selected')).toBe('true')
    const previewFrame = page.locator('iframe[title="report.html"]')
    await previewFrame.waitFor({ state: 'visible' })
    await expect.poll(async () => (await previewFrame.boundingBox())?.width ?? 0).toBeGreaterThanOrEqual(299)
    await expect.poll(() => page.frameLocator('iframe[title="report.html"]').getByRole('heading').innerText())
      .toBe('Preview ready')
    const screenshotPath = process.env.DSH_DESKTOP_PREVIEW_SCREENSHOT
