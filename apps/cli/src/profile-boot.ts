 ...
 * live when user layers are enabled, and wire fail-loud plus bounded shutdown.
 *
...
const NAME = 'dsh'
const MANAGED_HOT_PATCH_ENV = 'DSH_MANAGED_HOT_PATCH'
const MANAGED_HOT_STATUS_ENV = 'DSH_MANAGED_HOT_STATUS'

async function publishManagedPatchStatus(
  filename: string,
  statusPath: string,
  ok: boolean,
  error?: unknown,
): Promise<void> {
  const revision = createHash('sha256').update(await readFile(filename)).digest('hex')
  ...
  const stage = `${statusPath}.tmp-${randomUUID()}`
  await writeFile(stage, body, { encoding: 'utf8', mode: 0o600 })
  await rename(stage, statusPath)
}
...
interface ComposedProfile {
  profile: Profile
  /** Whether profile/home user patch files participate in this composition. */
  userPatches: boolean
...
function composeProfile(
  ...
  const profile = prepareProfile(name, userPatches)
  const homePatches = userPatches ? loadOptionalPatches(NAME, homePatchPath()) ?? [] : []
  ...
  return { profile, userPatches, bundlePatches, homePatches, overlays: composedOverlays, rows }
}

/**
 * Resolve whether locally editable profile/home patch layers are enabled.
 * Managed launchers set `DSH_MANAGED_PROFILE=1`; an explicit API option still
 * wins so embedders and tests can state the policy without mutating globals.
 * @param managedProfile - raw `DSH_MANAGED_PROFILE` launch value.
 * @param explicit - optional caller override.
 * @returns true when user patch files are part of the composition.
 */
export function resolveUserPatches(managedProfile: string | undefined, explicit?: boolean): boolean {
  return explicit ?? managedProfile !== '1'
}
...
export interface RunProfileOptions {
  ...
  /** Whether profile/home user patches are loaded and watched. Defaults off for a managed profile. */
  userPatches?: boolean
}
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  const composed = composeProfile(
    options.profile,
    options.patchFiles,
    resolveUserPatches(options.environment.get('DSH_MANAGED_PROFILE')?.value, options.userPatches),
  )
  ...
  const composeLive = (): PatchOptions[] => structuredClone([
    ...composed.bundlePatches,
    ...(composed.userPatches ? loadOptionalPatches(NAME, composed.profile.patchPath) ?? [] : []),
    ...(composed.userPatches ? loadOptionalPatches(NAME, homePatchPath()) ?? [] : []),
    ...composed.overlays,
  ])
  ...
  // loop drains. Managed compositions skip this block entirely because their
  // locally editable patch files are outside the launch policy.
  const ensureConfigHmr = async (): Promise<void> => {
    if (ctx.get('hmr') !== undefined) return
  ...
  const managedHotPatchValue = options.environment.get(MANAGED_HOT_PATCH_ENV)?.value
  const managedHotStatusValue = options.environment.get(MANAGED_HOT_STATUS_ENV)?.value
  if (managedHotPatchValue !== undefined && managedHotPatchValue.length > 0
    && !signalShutdown.signal.aborted
    && ctx.fiber.state === FiberState.ACTIVE
    && ctx.get('loader') !== undefined) {
    const managedHotPatch = resolve(managedHotPatchValue)
    const managedHotStatus = managedHotStatusValue === undefined || managedHotStatusValue.length === 0
      ? undefined
      : resolve(managedHotStatusValue)
    if (managedHotStatus === undefined) {
      throw new Error(`${NAME}: ${MANAGED_HOT_STATUS_ENV} is required when ${MANAGED_HOT_PATCH_ENV} is set`)
    }
    if (!options.patchFiles.map(file => resolve(file)).includes(managedHotPatch)) {
      throw new Error(`${NAME}: managed hot patch must also be passed through --patch`)
    }
    try {
      await ensureConfigHmr()
      await watchUserPatches(ctx, {
        binName: NAME,
        filename: managedHotPatch,
        compose: () => allPatches(composeProfile(options.profile, options.patchFiles, false)),
        onApplied: () => publishManagedPatchStatus(managedHotPatch, managedHotStatus, true),
        onFailed: error => publishManagedPatchStatus(managedHotPatch, managedHotStatus, false, error),
      })
    } catch (error) {
