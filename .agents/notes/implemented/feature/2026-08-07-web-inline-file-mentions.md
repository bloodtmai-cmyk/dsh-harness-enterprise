...
**The provider also owns the model guidance for its accepted syntax.** The ui-deliverables Node half registers a static `ui:deliverable-file-references` section that makes file delivery part of task completion. After any tool creates, modifies, converts, or exports a user-facing file, the model must call `deliver_files` in the same Turn for every final output, exclude temporary and intermediate files, wait for successful registration before claiming availability, and name each delivered file as Markdown inline code using its exact registered path or a basename only when unique within the Turn. The shipped Web patch is the only composition that loads ui-deliverables, so the guidance exists exactly where the renderer exists; removing the package removes both. The row remains the correctness path when the model omits a mention, and the matching vocabulary still rejects anything not recorded by a successful mutation.
