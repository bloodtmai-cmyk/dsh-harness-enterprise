...
import {
  ...
  DEFAULT_MAX_REQUEST_ATTEMPTS_PER_STEP,
...
/** Resolve one deployment-owned loop safety limit. */
function resolvePositiveSafeInteger(name: string, value: number | undefined, fallback: number): number {
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  static Config = z.object({
    ...
    maxRequestAttemptsPerStep: z.number().step(1).min(1).max(Number.MAX_SAFE_INTEGER).default(DEFAULT_MAX_REQUEST_ATTEMPTS_PER_STEP),
    maxTokensPerTurn: z.number().step(1).min(1).max(Number.MAX_SAFE_INTEGER).default(DEFAULT_MAX_TOKENS_PER_TURN),
  ...
  constructor(ctx: Context, config: Config) {
    ...
    this.config = {
      ...
      maxStepsPerTurn: resolvePositiveSafeInteger(
        'maxStepsPerTurn', config.maxStepsPerTurn, DEFAULT_MAX_STEPS_PER_TURN,
      ),
      maxRequestAttemptsPerStep: resolvePositiveSafeInteger(
        'maxRequestAttemptsPerStep',
        config.maxRequestAttemptsPerStep,
        DEFAULT_MAX_REQUEST_ATTEMPTS_PER_STEP,
      ),
      maxTokensPerTurn: resolvePositiveSafeInteger(
  ...
  private prepare(ownerCtx: Context, id: SessionId, options: AgentOptions, session: Session, callerSignal?: AbortSignal): PreparedAgent {
    ...
    try {
      const agent = machine = new ReactLoopAgent(loopCtx, id, options, session, {
        ...
        maxRequestAttemptsPerStep: this.config.maxRequestAttemptsPerStep,
