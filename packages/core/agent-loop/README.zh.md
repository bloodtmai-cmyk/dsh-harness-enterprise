...
通过配置创建的 agent 会自动启动。模型调用同时需要 `provider` 和 `model`；`agent/request` 可以在分发前补齐缺失的这一对值。可选的正数 `maxTokens` 会为每次对话请求提供初始输出上限，并记录在请求 header 中。`maxParallelToolCalls` 限制每个 agent 针对并行安全调用使用的滚动池，默认值为 `10`；它同时也是 `agent-loop` Settings 段的全部内容，因此叠加在该条目之上的用户层无需重启即可限制下一组工具调用，而非正整数的值会在写入时被拒绝，而不是到那一组时才失败。三个熔断字段属于部署策略，刻意不进入 Settings：当单轮的模型／工具步骤数、同一步提供方尝试次数或累计上报的输入／输出／缓存用量达到上限时，熔断器会在下一次提供方请求前结束该轮；新的用户轮次重新计数。若最终成功响应跨过 Token 上限，因为已经不需要下一次请求，仍会保留该响应。不上报用量的提供方仍受步骤与尝试次数保护。`agents` 同样不在 Settings 中——它在服务启动时被消费一次，所以存储的改动只会看起来生效。`cwd` 仅应用于全新会话，而 `resumeSessionId` 保留持久化元数据。通过配置创建的 agent 使用部署 persona；编程式 setup 可以按 agent 遮蔽它。该插件为每个 agent 提供 `provider`、`model` 和 `cwd` 提示词变量；harness 身份与部署 persona 属于 `dsh-system-prompt`。
...
熔断失败以 `AGENT_LOOP_CIRCUIT_OPEN` 关闭当前轮次。该错误在 `agent/request-error` 之外抛出，因此重试 middleware 不能重新打开被熔断器阻止的请求。已经完成的工具结果继续持久化，但循环不会开始下一个模型步骤。[失控轮次熔断决策](../../../.agents/notes/implemented/bug-fix/2026-08-19-runaway-turn-circuit-breaker.md)规定这些边界。
...
- **没有金额预算**：循环提供部署层拥有的步骤、请求尝试和上报 Token 上限，但不知道不同提供方的具体价格；提供方配额和金额预算仍由网关负责。
