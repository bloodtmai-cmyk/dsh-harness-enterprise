...
interface Config {
  ...
  maxStepsPerTurn?: number // default 64; deployment-owned turn circuit breaker
  maxRequestAttemptsPerStep?: number // default 8; includes the initial request
  maxTokensPerTurn?: number // default 2,000,000; cumulative reported usage
  agents: Array<{
    id: string                 // required
    provider?: string
    model?: string
...
Configured agents start automatically. A model call requires both `provider` and `model`; `agent/request` may supply a missing pair before dispatch. An optional positive `maxTokens` seeds each conversation request's output cap and is logged in its request header. `maxParallelToolCalls` bounds every agent's rolling pool for parallel-safe calls and defaults to `10`; it is also the whole of the `agent-loop` Settings section, so a user layer over this entry caps the next tool group without a restart, and a value that is not a positive integer is refused at the write rather than at that group. The three circuit-breaker fields are deployment policy and deliberately absent from Settings: they stop a turn before its next provider request when its model/tool steps, same-step provider attempts, or cumulative reported input/output/cache usage reaches the configured ceiling. A new user turn receives fresh counters. Successful terminal responses are preserved even if their final usage crosses the token ceiling because they require no next request. Usage-less providers remain protected by the step and attempt ceilings. `agents` is also absent from Settings — it is consumed once when the service starts, so a stored change could only look like it had an effect. `cwd` applies only to fresh sessions, while `resumeSessionId` retains persisted metadata. Configured agents use the deployment persona, and programmatic setup can shadow it per agent. This plugin supplies the per-agent `provider`, `model`, and `cwd` prompt variables; harness identity and deployment persona belong to `dsh-system-prompt`.
...
Circuit-breaker failures close the current turn as `AGENT_LOOP_CIRCUIT_OPEN`. They are raised outside `agent/request-error`, so retry middleware cannot reopen the request that the breaker stopped. Completed tool results remain durable, but the loop does not begin another model step. The [runaway-turn circuit-breaker decision](../../../.agents/notes/implemented/bug-fix/2026-08-19-runaway-turn-circuit-breaker.md) owns these boundaries.
...
- **No monetary budget** — the loop has deployment-owned step, request-attempt, and reported-token ceilings, but it does not know provider-specific prices. Provider quota and currency budgets remain the gateway's responsibility.
