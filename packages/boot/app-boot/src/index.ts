...
export interface UserPatchWatchOptions {
  ...
  /** Called only after the candidate tree committed successfully. */
  onApplied?: () => Promise<void> | void
  /** Called before a failed candidate is rethrown to HMR diagnostics. */
...
export async function watchUserPatches(
  ...
  const { binName, filename, compose = (patches: PatchOptions[]) => patches, onApplied, onFailed } = options
