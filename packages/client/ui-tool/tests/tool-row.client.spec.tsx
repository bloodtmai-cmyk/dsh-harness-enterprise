...
import { classifyTool, resultChart, resultText, toolRowModel } from '../src/client/tool/models/tool-call-model.ts'
...
describe('tool-call-model', () => {
  ...
  it('keeps chart payload out of flattened text and exposes it to the rich renderer', () => {
    const chart = {
      type: 'chart' as const,
      chartType: 'line',
      payload: { type: 'line', data: [{ time: '1月', value: 10 }] },
    }
    const node = result({ content: [{ type: 'text', text: '折线图已生成。' }, chart] })
    expect(resultText(node)).toBe('折线图已生成。')
    expect(resultChart(node)).toBe(chart)
  })
...
describe('ToolRow', () => {
  ...
  it('renders leading icon, title and summary while collapsed', () => {
    ...
    expect(view.container.querySelector('[data-stream-step="tool"]')).not.toBeNull()
