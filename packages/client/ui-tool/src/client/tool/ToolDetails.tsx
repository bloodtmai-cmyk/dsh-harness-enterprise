...
import { resultChart, resultText } from './models/tool-call-model.ts'
import { GptVisChart } from './components/GptVisChart.tsx'
...
export function ToolDetails({ block, cwd, t }: ToolDetailsContentProps) {
  ...
  const chart = resultChart(block)
  const text = resultText(block)
  if (chart !== undefined) {
    return (
      <>
        <GptVisChart chart={chart} t={t} />
        {text !== '' ? <pre className={css.code}>{text}</pre> : null}
      </>
    )
  }
  return (
