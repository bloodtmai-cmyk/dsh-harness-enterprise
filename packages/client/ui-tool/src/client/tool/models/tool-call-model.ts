...
export function resultText(node: ToolResultNode): string {
  ...
  for (const block of node.content) {
    ...
    else if (block.type === 'chart') continue
...
/** Return the first validated chart block attached to a settled Tool result. */
export function resultChart(
  node: ToolResultNode,
): Extract<ToolResultNode['content'][number], { type: 'chart' }> | undefined {
  return node.content.find(
    (block): block is Extract<ToolResultNode['content'][number], { type: 'chart' }> => block.type === 'chart',
  )
}
