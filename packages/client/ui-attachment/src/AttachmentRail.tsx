...
export interface AttachmentRailItem {
  ...
  previewUrl?: string
  /** Display name used by non-image file cards. */
  fileName?: string
...
export interface AttachmentRailLabels {
  ...
  open: string
  /** Tooltip and accessible name for opening a non-image file. */
  openFile: string
        ...
        {items.map(item => (
          <div key={item.id} className={css.item}>
            <button
              ...
              title={item.previewUrl === undefined ? labels.openFile : labels.open}
              aria-label={item.previewUrl === undefined ? `${labels.openFile} ${item.fileName ?? item.alt}` : undefined}
              onClick={() => { onOpen(item) }}
            >
              {item.previewUrl === undefined ? (
                <span className={css.document} aria-label={item.alt}>
