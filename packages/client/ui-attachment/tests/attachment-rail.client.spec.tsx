...
describe('AttachmentRail', () => {
  ...
  it('opens document cards instead of leaving them inert', () => {
    const document = {
      id: 'doc', fileName: 'report.xlsx', alt: 'report.xlsx', removeLabel: '移除附件 report.xlsx',
    }
    const onOpen = vi.fn()
    const view = render(<AttachmentRail items={[document]} labels={labels} onOpen={onOpen} onRemove={vi.fn()} />)
    fireEvent.click(view.getByRole('button', { name: '打开文件 report.xlsx' }))
