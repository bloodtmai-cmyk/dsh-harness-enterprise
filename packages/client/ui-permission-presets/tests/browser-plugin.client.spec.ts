...
describe('ui-permission browser plugin', () => {
  ...
  it('availability follows the projection key; options mark the current value active and exclude custom', async () => {
    ...
    expect(again.find(option => option.id === 'read-only')?.detail).toBe(accessEn['preset.description.readOnly'])
    // Built-in names are localized; non-kebab host-configured names pass through.
