...
describe('BrandWordmark', () => {
  it('pairs the whale with Harness Enterprise branding', () => {
    const { container } = render(<primitives.BrandWordmark />)
    const root = container.querySelector<HTMLElement>('[data-brand-wordmark]')!
    expect(root.style.width).toBe('83px')
    expect(root.style.height).toBe('24px')
    expect(root.style.gap).toBe('10px')
    expect(root.children).toHaveLength(2)
    expect(root.textContent).toContain('HARNESS')
    expect(container.innerHTML.toLowerCase()).not.toContain('deepseek')
    expect(container.innerHTML).toContain('currentColor')
  })
})
