...
/** User-facing copy supplied by the locale-owning render site. */
...
interface WebBlockCommon {
  /** Localized copy; defaults preserve the original Simplified Chinese surface. */
  labels?: WebBlockLabels | undefined
}

/** A `web_search` card: an optional answer over a capped citation list. */
export interface WebSearchBlockProps extends WebBlockCommon {
  kind: 'search'
...
export interface WebFetchBlockProps extends WebBlockCommon {
  kind: 'fetch'
 ...
 */
function WebSearchBlock({ answer, sources, truncated, className, labels = DEFAULT_LABELS }: WebSearchBlockProps) {
 ...
 */
function WebFetchBlock({ url, statusCode, truncated, className, labels = DEFAULT_LABELS }: WebFetchBlockProps) {
  return (
