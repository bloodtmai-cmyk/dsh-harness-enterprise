...
export interface SearchBlockLabels {
  ...
  shown: (retained: number, total: number) => string
...
const DEFAULT_LABELS: SearchBlockLabels = {
  ...
  shown: (retained, total) => `显示 ${retained} / 共 ${total}`,
...
function summaryText(
  ...
  const count = truncated ? labels.shown(shown, total) : `${shown}`
  return props.kind === 'paths'
    ? labels.paths(count)
...
export function SearchBlock(props: SearchBlockProps) {
  const { truncated, total, maxLines = DEFAULT_SEARCH_MAX_LINES, className, labels = DEFAULT_LABELS } = props
