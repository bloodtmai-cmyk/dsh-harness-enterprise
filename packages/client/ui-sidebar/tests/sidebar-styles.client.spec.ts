...
describe('SidebarRoot.module.css', () => {
  ...
  it('keeps compact update controls beside Settings without reserving an empty seat', () => {
    expect(declarations('.settingsRow')?.get('display')).toBe('flex')
    expect(declarations('.settingsActions')?.get('min-width')).toBe('36px')
    expect(declarations('.collapsed .settingsRow')?.get('flex-direction')).toBe('column-reverse')
    expect(declarations('.settingsActions:not(:has(button))')?.get('display')).toBe('none')
  })
})
