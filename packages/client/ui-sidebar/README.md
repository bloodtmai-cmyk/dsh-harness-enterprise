...
`SidebarRootComponentProps` composes the layout owner share, the global `useSessions` and `useWorkspaces` hooks, the declared `sidebar.workspaces`, `sidebar.settings`, `sidebar.settings.action`, and `sidebar.footer.action` child slots, and injected `startSession` plus sidebar-toggle callbacks. There is no plugin store.
...
The foot keeps full-width `sidebar.footer.action` destinations above the bottom-pinned `sidebar.settings` seat. Compact `sidebar.settings.action` status controls sit beside Settings when expanded and immediately above it in the 56px rail. Every occupant receives only the column state (`wide`).
