...
`SidebarRootComponentProps` 组合布局 owner share、全局 `useSessions` 和 `useWorkspaces` 钩子、已声明的 `sidebar.workspaces`、`sidebar.settings`、`sidebar.settings.action` 与 `sidebar.footer.action` 子 slot，以及注入的 `startSession` 与侧边栏切换回调。这里没有插件 store。
...
页脚把全宽的 `sidebar.footer.action` 入口放在固定到底部的 `sidebar.settings` 上方。紧凑的 `sidebar.settings.action` 状态控件在展开时位于 Settings 旁边，在 56px 轨道里则紧邻其上方；每个占位只接收栏状态（`wide`）。
