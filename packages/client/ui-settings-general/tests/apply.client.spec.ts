...
describe('ui-settings-general apply', () => {
  ...
  it('registers About only when the desktop preload exposes application identity', async () => {
    vi.stubGlobal('window', {
      desktopAppInfo: { productName: 'Harness Enterprise Desktop', version: '0.1.0-rc.24' },
    })
    try {
      const b = await bench()
      declare(b.slots)
      const fiber = b.ctx.plugin({ inject: [...inject], apply })
      await fiber.await()
      const entry = b.slots.entries('settings.section').find(e => e.component === AboutSection)!
      expect(entry.options).toMatchObject({ id: 'about', order: 100 })
      expect(resolveSlotLabel(entry.options.label)).toBe('关于')
      expect((entry.inject as unknown as () => AboutSectionInjected)()).toMatchObject({
        info: { productName: 'Harness Enterprise Desktop', version: '0.1.0-rc.24' },
      })
      await fiber.dispose()
      expect(b.slots.entries('settings.section').find(e => e.component === AboutSection)).toBeUndefined()
    } finally {
      vi.unstubAllGlobals()
    }
  })
