...
详情壳也接受可选的 Electron 桌面预览桥。桥存在时，第三列显示 `详情 / 预览` 两个标签；`openFile` 会把支持的产出文件和行内文件链接作为带当前会话工作目录的预览请求交给该列，并自动展开它。桥缺席或文件类型不支持时仍调用原有 `workspaces.openPath`，因此普通 Web 组装没有桌面 API 依赖，也不改变既有文件打开行为。真正的读取、路径权限与大小限制属于桌面主进程，本包只拥有标签、事件路由和渲染面。
...
- **工具调用详情仍没有入口**：Electron 产出文件可以打开同一个右侧壳的预览标签，但 `ChatViewInjected.openDetails` 仍无人调用，因此原始工具调用详情在组装后的应用中不可达。没有 Input/Output/Metadata 切换、Prev/Next 步进，也没有 trajectory 深链接。
