...
import type {
  DetailsPanelOwnerProps, DetailsSlotProps, DetailsTabOwnerProps, DetailsToolOwnerProps, SelectionTarget,
...
function renderToolDetailsProbe(owners?: DetailsToolOwnerProps[]): DetailsSlotProps['renderSlot'] {
  return ((key: string, owner: unknown, options?: { fallback?: ReactNode }) => {
...
describe('render branch tails', () => {
  ...
  it('DetailsPanel lets an enterprise extension contribute a tab and routed panel body', () => {
    localStorage.clear()
    const snap = snapshotBase()
    const chat = createChatStore().create()
    const emptyList = createSnapshotStore<SessionListState>(
    ...
    const renderSlot = ((key: string, owner: unknown, options?: { fallback?: ReactNode }) => {
      if (key === 'conversation.details.tabs') {
        const tabs = owner as DetailsTabOwnerProps
        return <button type="button" onClick={() => { tabs.selectPane('enterprise', '企业面板') }}>企业面板</button>
      }
      if (key === 'conversation.details.panel'
        && (owner as DetailsPanelOwnerProps).activePane === 'enterprise') {
        return <div>受管扩展内容</div>
      }
    ...
    fireEvent.click(view.getByText('企业面板'))
    expect(view.getByText('受管扩展内容')).toBeTruthy()
    expect(view.getAllByText('企业面板')).toHaveLength(2)
  })
