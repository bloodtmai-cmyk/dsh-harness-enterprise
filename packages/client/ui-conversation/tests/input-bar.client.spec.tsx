...
describe('attachment draft rail', () => {
  ...
  it('accepts a drop anywhere on the page under the full-page overlay', () => {
    ...
    expect(view.getByRole('status').textContent).toContain('拖动图片或 Office 文件到此处')
  ...
  it('passes an Office document from a page-wide drop to the attachment service', () => {
    const addImages = vi.fn(() => null)
    bench({ addImages })
    const workbook = new File([Uint8Array.of(0x50, 0x4b, 0x03, 0x04)], '库存分析.xlsx', {
      type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
    })
    fireEvent.drop(document.body, {
      dataTransfer: { types: ['Files'], files: [workbook], dropEffect: 'none' },
    })
    expect(addImages).toHaveBeenCalledWith([workbook])
  })

  it('opens a pending Office attachment through the desktop bridge', async () => {
    const workbook = new File([Uint8Array.of(0x50, 0x4b, 0x03, 0x04)], '库存分析.xlsx', {
      type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
    })
    const attachment: ComposerAttachment = {
      kind: 'document', id: 'doc-1' as DraftAttachmentId, file: workbook,
    }
    ...
    fireEvent.click(view.getByRole('button', { name: '打开 Office 文件 库存分析.xlsx' }))
  ...
  it('shows the projected limits in the drop overlay desc line', () => {
    ...
    expect(view.getByRole('status').textContent).toContain('支持图片、Word、Excel、PowerPoint')
  })
