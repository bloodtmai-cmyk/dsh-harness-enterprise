...
import type {
  AssistantMessageNode, CommandNode, CompactionSummaryNode, ContextMessageNode, ConversationNode, ConversationSnapshot,
...
function makeHarness(init?: Partial<ConversationSnapshot>) {
  ...
  const props: ChatViewSlotProps = {
    ...
    loadDocument: vi.fn(() => Promise.reject(new Error('not used'))),
    inspectCall,
    chatScroll,
    forkAt,
...
describe('Chat node rendering', () => {
  ...
  it('opens a historical Office attachment through the session loader and desktop bridge', async () => {
    const attachment = {
      attachmentId: 'doc-history' as never,
      mediaType: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' as const,
      bytes: 3,
      name: 'report.docx',
    }
    const documentUser: UserMessageNode = {
      ...user(1, ''),
      content: [{ type: 'document', attachment, text: 'report', truncated: false }] as never,
    }
    const h = makeHarness({ nodes: [documentUser] })
    const loadDocument = vi.fn(() => Promise.resolve(new Uint8Array([1, 2, 3])))
    const open = vi.fn((_request: { name: string; mediaType: string; data: Uint8Array }) => (
      Promise.resolve({ ok: true })
    ))
    h.props.loadDocument = loadDocument
    window.desktopAttachments = { open }
    const view = render(<h.ChatView {...h.props} />)

    fireEvent.click(view.getByRole('button', { name: '打开 report.docx' }))
    await waitFor(() => { expect(open).toHaveBeenCalledTimes(1) })
    expect(loadDocument).toHaveBeenCalledWith(attachment)
  })
...
describe('ChatView', () => {
  ...
  it('renders loop circuit-breaker failures as localized safety guidance', () => {
    const h = makeHarness({ nodes: [user(1, 'try'), turnError(2, 'AGENT_LOOP_CIRCUIT_OPEN')] })
    const view = render(<h.ChatView {...h.props} />)

    expect(view.getByRole('status').textContent).toBe(
      '已停止可能的循环执行本轮已达到企业安全上限。请缩小任务范围或明确完成条件后重试。AGENT_LOOP_CIRCUIT_OPEN',
    )
  ...
  it('keeps agent work visible while running, then folds it behind one process disclosure', () => {
    const midStep: AssistantMessageNode = {
      ...assistant(2, 'working note'),
      step: 1,
      blocks: [
        { kind: 'reasoning', text: 'Inspecting sources' },
        { kind: 'text', text: 'working note' },
      ],
    }
    const closingStep: AssistantMessageNode = {
      ...assistant(4, 'final answer'),
      step: 2,
      blocks: [
        { kind: 'reasoning', text: 'Summarizing evidence' },
        { kind: 'text', text: 'final answer' },
      ],
    }
    const h = makeHarness({
      nodes: [user(1, 'question'), midStep, closingStep],
      running: true,
    })
    const view = render(<h.ChatView {...h.props} />)

    expect(view.queryByRole('button', { name: /思考过程/ })).toBeNull()
    expect(view.getByText('working note')).toBeTruthy()
    expect(view.getByText('Summarizing evidence')).toBeTruthy()

    act(() => {
      h.set({ running: false, turnEnds: new Map([[1, 5]]) })
    })

    const process = view.getByRole('button', { name: /思考过程.*2 个步骤/ })
    expect(view.getByText('final answer')).toBeTruthy()
    expect(view.queryByText('working note')).toBeNull()
    expect(view.queryByText('Summarizing evidence')).toBeNull()

    fireEvent.click(process)
    expect(view.getByText('working note')).toBeTruthy()
    expect(view.getByText('Inspecting sources')).toBeTruthy()
    expect(view.getByText('Summarizing evidence')).toBeTruthy()
  ...
  it('folds turn-local context and Tool rows into the same settled process disclosure', () => {
    const context: ContextMessageNode = {
      kind: 'context',
      seq: 2,
      time: 2_000,
      content: [{ type: 'text', text: 'catalog contents' }] as never,
      source: { kind: 'skill-catalog' },
      provenance: { role: 'inject', label: 'skill-catalog' },
      form: null,
    }
    const midStep = { ...assistant(3, 'researching'), step: 1 }
    const closingStep: AssistantMessageNode = {
      ...assistant(5, 'settled result'),
      step: 2,
      blocks: [
        { kind: 'reasoning', text: 'Checking the result' },
        { kind: 'text', text: 'settled result' },
      ],
    }
    const base = chatSnapshotFixture({
      nodes: [user(1, 'question'), context, midStep, toolResult(4, 'search'), closingStep],
      turnEnds: new Map([[1, 6]]),
    })
    const turn = base.timeline.turns.get(1)
    expect(turn).toBeDefined()
    const values = base.nodes.values().map(node => node.kind === 'context' || node.kind === 'tool-call'
      ? { ...node, location: { kind: 'turn' as const, turn: turn! } }
      : node)
    ...
    const process = view.getByRole('button', { name: /思考过程.*4 个步骤/ })
    expect(view.getByText('settled result')).toBeTruthy()
    expect(view.queryByText('skill-catalog')).toBeNull()
    expect(view.queryByTestId('tool-seat-search')).toBeNull()

    fireEvent.click(process)
    expect(view.getByRole('button', { name: /上下文注入.*skill-catalog/ })).toBeTruthy()
    expect(view.getByTestId('tool-seat-search')).toBeTruthy()
    expect(view.getByText('Checking the result')).toBeTruthy()
  })
  ...
  it('hands running calls to a live Tool group', () => {
    ...
    expect(view.container.querySelector('[data-stream-activity-surface]')).not.toBeNull()
    expect(view.container.querySelector('[data-chat-flow][data-streaming="true"]')).not.toBeNull()
  })
  ...
  it('the running clock uses turn/start, ignores steering, and stays out of the live region', () => {
    ...
    expect(status.textContent).toMatch(/^正在处理2分0\d秒$/)
