...
} from './skeleton/DesktopFilePreview.tsx'
...
export function apply(ctx: Context): void {
      ...
      return {
        ...
        addImages: (files) => {
          try {
            ...
            if (error instanceof UnsupportedAttachmentMediaTypeError) return t('attachment.unsupportedType')
    ...
    inject: (sessionId: SessionId, actions: BoundActions<typeof chatStore>): ChatViewInjected => {
      ...
      return {
        ...
        openFile: (path) => {
          ...
          if (cwd !== undefined && requestDesktopFilePreview({ path: resolvedPath, workspaceRoot: cwd })) {
            layout.openDetails()
            return
          }
          void workspaces.openPath(resolvedPath).catch(() => {
        ...
        loadDocument: attachment => conversation.resolveDocument(sessionId, attachment),
  ...
  slots.register({
    ...
    children: {
      ...
      'conversation.details.tabs': { kind: 'list', scope: 'session' },
      'conversation.details.panel': { kind: 'keyed', scope: 'session' },
    },
    store: chatStore,
