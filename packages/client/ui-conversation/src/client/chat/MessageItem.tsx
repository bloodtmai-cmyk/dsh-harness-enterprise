...
import { IconPaperclipOutline16, JsonBlock, MessageText, StateDot } from '@deepseek-ai/dsh-client-ui-primitives'
import type { ChatNodeViewProps, ChatViewSlotProps } from '../contract/slots.ts'
import { ImageGallery, type ImageLoader } from '@deepseek-ai/dsh-client-ui-attachment'
import { messageImageLabels } from '../image-labels.ts'
import { openStoredDocument } from '../desktop-attachment.ts'
import type { DocumentLoader } from '../desktop-attachment.ts'
...
function contentParts(content: readonly unknown[]): {
  ...
  for (const block of content) {
    ...
    else if (b.type === 'document' && b.attachment !== undefined) documents.push(b as UserDocument)
...
function TurnErrorItem({ node, t }: {
  ...
  const circuitOpen = node.code === 'AGENT_LOOP_CIRCUIT_OPEN'
  return (
    <div className={css.turnErrorRow} role="status">
      <StateDot state="error" className={css.turnErrorDot} />
      <div className={css.turnErrorCopy}>
        <span className={css.turnErrorTitle}>{t(circuitOpen ? 'message.circuitOpen' : 'message.turnError')}</span>
        <span className={css.turnErrorMessage}>{circuitOpen ? t('message.circuitOpen.hint') : node.message}</span>
      </div>
...
function UserStyleBubble({
  content, imageLoader, documentLoader, actions, pending = false, t,
}: {
  ...
  return (
    <div className={css.userRow} data-pending-steering={pending || undefined} data-time-hover-root>
      <div className={css.userStack}>
        ...
        {documents.length > 0 && (
          <div className={css.documents} role="list">
            {documents.map(document => (
              <div role="listitem" key={String(document.attachment.attachmentId)}>
                <button
                  type="button"
                  className={css.documentCard}
                  aria-label={t('document.openNamed', { name: document.attachment.name })}
                  onClick={() => { void openStoredDocument(document.attachment, documentLoader).catch(() => {}) }}
                >
                  <span className={css.documentIcon}><IconPaperclipOutline16 /></span>
                  <span className={css.documentMeta}>
                    <span className={css.documentName}>{document.attachment.name}</span>
                    <span className={css.documentSize}>
                      {Math.max(1, Math.ceil(document.attachment.bytes / 1024))} KB
                      {document.truncated ? ` · ${t('document.extractedTruncated')}` : ''}
                    </span>
                  </span>
                </button>
              </div>
 ...
 */
export function PendingSteeringBubble({ content, loadImage, loadDocument, t }: {
  content: readonly unknown[]
  ...
  const documentLoader = loadDocument ?? (() => Promise.reject(new Error('document loading service unavailable')))
  return (
    <UserStyleBubble
      content={content}
      imageLoader={imageLoader}
