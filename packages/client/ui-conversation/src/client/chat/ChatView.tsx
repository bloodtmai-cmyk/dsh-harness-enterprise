...
import {
  DisclosureRow, IconChevronDownOutline14, IconThinkOutline14,
...
interface ChatNodeReader {
  get: (key: string) => unknown
}

interface TurnProcessGroup {
  readonly key: string
  readonly turn: number
  readonly insertAt: number
  readonly anchorKey: string
  readonly nodeKeys: readonly string[]
  readonly closingReasoning: readonly AssistantBlock[]
  readonly stepCount: number
}

interface TurnProcessLayout {
  readonly groupsByIndex: ReadonlyMap<number, TurnProcessGroup>
  readonly hiddenNodeKeys: ReadonlySet<string>
  readonly answerOnlyNodeKeys: ReadonlySet<string>
}

function chatNodeAt(nodes: ChatNodeReader, key: string): ChatNode | undefined {
  return nodes.get(key) as ChatNode | undefined
}

function nodeTurn(node: ChatNode): number | undefined {
...
/** Fold completed agent work while leaving the closing answer in the main flow. */
function turnProcessLayout(order: readonly string[], nodes: ChatNodeReader): TurnProcessLayout {
  const groupsByIndex = new Map<number, TurnProcessGroup>()
  const hiddenNodeKeys = new Set<string>()
  const answerOnlyNodeKeys = new Set<string>()
  const indexed = order.map((key, index) => ({ key, index, node: chatNodeAt(nodes, key) }))

  for (const entry of indexed) {
    if (entry.node?.kind !== 'turn-tail' || entry.node.data.closing === null) continue
    const turn = entry.node.data.turn
    const closingSeq = entry.node.data.closing.finalNode.seq
    const closing = indexed.findLast(candidate => candidate.node?.kind === 'assistant-step'
      && nodeTurn(candidate.node) === turn
      && candidate.node.data.finalNode?.seq === closingSeq)
    if (closing?.node?.kind !== 'assistant-step') continue

    const process = indexed.filter(candidate => candidate.key !== closing.key
      && candidate.node !== undefined
      && nodeTurn(candidate.node) === turn
      && isProcessNode(candidate.node))
    const closingReasoning = closing.node.data.blocks.filter(block => block.kind === 'reasoning')
    if (process.length === 0 && closingReasoning.length === 0) continue

    const insertAt = Math.min(
      process[0]?.index ?? closing.index,
      closingReasoning.length > 0 ? closing.index : Number.POSITIVE_INFINITY,
    )
    const nodeKeys = process.map(candidate => candidate.key)
    for (const key of nodeKeys) hiddenNodeKeys.add(key)
    if (closingReasoning.length > 0) answerOnlyNodeKeys.add(closing.key)
    groupsByIndex.set(insertAt, {
      key: `turn-process:${turn}:${entry.key}`,
      turn,
      insertAt,
      anchorKey: nodeKeys[0] ?? closing.key,
      nodeKeys,
      closingReasoning,
      stepCount: nodeKeys.length + closingReasoning.length,
    })
  }
  return { groupsByIndex, hiddenNodeKeys, answerOnlyNodeKeys }
}

function TurnProcessDisclosure({
  group, useSession, selectedCallId, cwd, openFile, inspectCall, forkAt,
  loadImage, loadDocument, fileMentions, renderSlot, t,
}: {
  readonly group: TurnProcessGroup
} & Pick<ChatViewSlotProps,
  'useSession' | 'openFile' | 'inspectCall' | 'forkAt' | 'loadImage' | 'loadDocument' | 'fileMentions' | 'renderSlot' | 't'> & {
  ...
  return (
    <div
      className={`${css.flowItem} ${css.processGroup}`}
      data-chat-anchor-key={group.anchorKey}
      data-chat-flow-key={group.key}
      data-chat-flow-kind="turn-process"
      data-chat-process-turn={group.turn}
      data-stream-process="settled"
    >
      <DisclosureRow
        className={css.processDisclosure}
        rowClassName={css.processRow}
        leadingClassName={css.processLeading}
        titleClassName={css.processTitle}
        ...
        collapsedContent={(
          <>
            <span className={css.processSeparator} aria-hidden />
            <span className={css.processSummary}>{t('chat.processSteps', { count: group.stepCount })}</span>
          </>
        )}
      >
        <div className={css.processBody} data-stream-process-body>
          {group.nodeKeys.map(nodeKey => (
            <ChatNodeSeat
              key={nodeKey}
              nodeKey={nodeKey}
          ...
          {group.closingReasoning.length > 0 && (
            <AssistantMarkdown
              blocks={group.closingReasoning}
              streaming={false}
              loadImage={loadImage}
              t={t}
            />
...
function TurnStatus({ startTime, t }: {
  ...
  return (
    <div className={css.turnStatus} role="status" aria-live="polite" data-stream-turn-status>
      <IconThinkOutline14 aria-hidden="true" />
      {t('chat.activity')}
      {showClock && (
...
export function ChatView({
  useSession, useSessions, useStore, renderSlot, sessionId, openFile, loadOlder, loadImage, loadDocument, inspectCall, chatScroll, forkAt,
  fileMentions, t,
}: ChatViewSlotProps) {
  ...
  const processLayout = useMemo(() => turnProcessLayout(order, nodeStore), [nodeStore, order])
  ...
  return (
    <div className={css.root} data-stream-activity-surface>
      <div ref={listRef} className={css.scroll}>
        ...
        >
          ...
          {order.map((nodeKey, index) => {
            const processGroup = processLayout.groupsByIndex.get(index)
            const hidden = processLayout.hiddenNodeKeys.has(nodeKey)
            return (
              <Fragment key={nodeKey}>
                ...
                {!hidden && (
                  <ChatNodeSeat
                    ...
                    loadDocument={loadDocument}
                    fileMentions={fileMentions}
                    assistantContent={processLayout.answerOnlyNodeKeys.has(nodeKey) ? 'answer' : 'all'}
                    renderSlot={renderSlot}
                    t={t}
                  />
                )}
