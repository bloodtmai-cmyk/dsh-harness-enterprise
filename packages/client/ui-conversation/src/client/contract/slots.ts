...
export interface ComposerAttachment {
  ...
  file: File
  /** Image-only object URL; documents use a file card instead of a bitmap preview. */
  previewUrl?: string
}
...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    ...
    /** Additional tab commands contributed to the right-side workspace panel. */
    'conversation.details.tabs': { kind: 'list'; scope: 'session'; owner: DetailsTabOwnerProps }
    /** Pane-id keyed bodies for contributed right-side workspace tabs. */
    'conversation.details.panel': { kind: 'keyed'; scope: 'session'; owner: DetailsPanelOwnerProps }
    /**
...
export interface ChatNodeOwnerProps {
  ...
  /** Resolve one session-authorized Office attachment for native open/download. */
  loadDocument: (attachment: DocumentAttachmentRef) => Promise<Uint8Array>
  fileMentions: (owner: TurnTailOwnerProps) => MarkdownFileMentions | undefined
  /** Internal split used by the settled-turn process disclosure. */
  assistantContent?: 'all' | 'answer' | undefined
}
...
/** Stable control surface for an extension tab in the right-side workspace panel. */
export interface DetailsTabOwnerProps {
  activePane: string
  /** Select an extension pane and optionally provide its human-readable header title. */
  selectPane: (pane: string, title?: string) => void
}

/** Routing currency for an extension-owned right-side workspace panel body. */
...
export interface ComposerBarInjected {
  ...
  /** Create draft attachments and append their ids to the session input. */
...
export type DetailsSlotProps = PropsRuntime<'details'>
  & PropsRenderSlots<'conversation.details.tool' | 'conversation.details.tabs' | 'conversation.details.panel'>
