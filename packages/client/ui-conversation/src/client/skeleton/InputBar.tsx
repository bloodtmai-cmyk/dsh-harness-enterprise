...
import { openDraftDocument } from '../desktop-attachment.ts'
...
export function InputBar({
  ...
  const intakeImages = useCallback((files: readonly File[]): void => {
    ...
    const rejected = ((): string | null => {
      if (imageLimits !== undefined) {
        ...
        const documents = files.filter(file => (documentLimits?.mediaTypes as readonly string[] | undefined)?.includes(file.type) ?? false)
        ...
        const existingDocuments = attachments.filter(attachment => attachment.kind === 'document')
        if (existingImages.length + images.length > imageLimits.maxImagesPerMessage) {
          return t('image.tooMany', { count: imageLimits.maxImagesPerMessage })
        }
        if (images.some(file => file.size > imageLimits.maxImageBytes)) {
          return t('image.fileTooLarge', { size: imageSizeText(imageLimits.maxImageBytes) })
        }
        const imageTotal = existingImages.reduce((sum, attachment) => sum + attachment.file.size, 0)
          + images.reduce((sum, file) => sum + file.size, 0)
        if (imageTotal > imageLimits.maxMessageImageBytes) {
          return t('image.totalTooLarge', { size: imageSizeText(imageLimits.maxMessageImageBytes) })
        }
        if (documentLimits !== undefined) {
          if (existingDocuments.length + documents.length > documentLimits.maxDocumentsPerMessage) {
          ...
          const documentTotal = existingDocuments.reduce((sum, attachment) => sum + attachment.file.size, 0)
            + documents.reduce((sum, file) => sum + file.size, 0)
          if (documentTotal > documentLimits.maxMessageDocumentBytes) {
  ...
  const railItems = useMemo<ComposerRailItem[]>(() => attachments.map(attachment => ({
    ...
    ...(attachment.kind === 'document' ? { fileName: attachment.file.name } : {}),
    alt: attachment.file.name || t('image.pending'),
    removeLabel: t(attachment.kind === 'image' ? 'image.remove' : 'attachment.remove', { name: attachment.file.name }),
    attachment,
  })), [attachments, t])
          ...
          <div className={css.attachments}>
            <AttachmentRail
              ...
              onOpen={(item) => {
                ...
                else void openDraftDocument(item.attachment.file).catch(() => {})
              }}
