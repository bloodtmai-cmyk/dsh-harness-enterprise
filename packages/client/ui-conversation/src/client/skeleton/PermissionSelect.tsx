...
import { accessModeDescription, accessModeLabel } from '../presentation-copy.ts'
...
function optionLabel(option: PermissionSelectValue['options'][number], t: ComposerBarProps['t']): string {
  return accessModeLabel(option.value, displayName(option.name), t)
}
          ...
          <button
            ...
            aria-label={t('input.accessMode', {
              name: current === undefined
                ? accessModeLabel(currentValue, displayName(currentValue), t)
                : optionLabel(current, t),
            })}
            title={current === undefined
              ? undefined
              : accessModeDescription(current.value, current.description, t)}
