...
import type { DocumentAttachmentRef, DocumentMediaType, ImageAttachmentRef, ImageMediaType } from '@deepseek-ai/dsh-attachment'
...
export class UnsupportedAttachmentMediaTypeError extends UnsupportedImageMediaTypeError {
  constructor(mediaType: string) {
    super(mediaType)
...
export class ConversationController extends Service implements IConversation {
  ...
  createDraftImages(files: readonly File[]): readonly ComposerAttachment[] {
    for (const file of files) attachmentKind(file)
  ...
  resolveImage(sessionId: SessionId, attachment: ImageAttachmentRef): Promise<string> {
    ...
    const pending = session.readAttachment(attachment.attachmentId)
      .then((result) => {
        ...
        if (!('width' in result.value.attachment)
          || result.value.attachment.attachmentId !== attachment.attachmentId
          || result.value.attachment.mediaType !== attachment.mediaType
          || result.value.attachment.bytes !== attachment.bytes
          || result.value.attachment.width !== attachment.width
          || result.value.attachment.height !== attachment.height
          || result.value.attachment.name !== attachment.name) {
          throw new Error('conversation.resolveImage: attachment metadata mismatch')
        }
  ...
  /** Read one session-authorized durable Office attachment for native open/download. */
  async resolveDocument(sessionId: SessionId, attachment: DocumentAttachmentRef): Promise<Uint8Array> {
    if (this.disposed) throw new Error('conversation.resolveDocument: service is disposed')
    const session = this.requireSessions().binding(sessionId)?.session
    if (session === undefined) throw new Error(`conversation.resolveDocument: unknown session "${sessionId}"`)
    ...
    if ('width' in result.value.attachment
      ...
      || result.value.attachment.bytes !== attachment.bytes
      || result.value.attachment.name !== attachment.name) {
      throw new Error('conversation.resolveDocument: attachment metadata mismatch')
    }
    return Uint8Array.from(result.value.data)
  }
  ...
  private serializeAttachments(attachments: readonly ComposerAttachment[]): Promise<Parameters<SessionFace['prompt']>[0]> {
    return Promise.all(attachments.map(async ({ file, kind }) => kind === 'image'
      ? {
        type: 'image' as const,
      ...
      : {
        ...
        mediaType: documentMediaType(file.type, file.name),
        data: bytesToBase64(new Uint8Array(await file.arrayBuffer())),
        name: file.name,
      }))
  }
...
function documentMediaType(value: string, name = ''): DocumentMediaType {
  switch (value) {
    ...
    default:
      switch (name.toLowerCase().split('.').pop()) {
        ...
        default: throw new UnsupportedAttachmentMediaTypeError(value)
      }
  }
}
...
function attachmentKind(file: File): ComposerAttachment['kind'] {
  try {
    ...
    if (!(error instanceof UnsupportedImageMediaTypeError)) throw error
    documentMediaType(file.type, file.name)
    return 'document'
  }
}
