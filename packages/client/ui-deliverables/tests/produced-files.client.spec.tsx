...
import {
  basename, deliverablesDefinition, parentDirectory, producedFileMentions, producedForClosing, selectProducedFiles,
...
describe('ProducedFiles row', () => {
  ...
  it('keeps one measured line, updates on resize, and opens a file or its folder', () => {
    ...
    expect(openFile).toHaveBeenLastCalledWith('deep')
  ...
  it('shows the folder action without overflow and hides it without a local native opener', () => {
    const openFile = vi.fn<(path: string) => void>()
    const view = render(
...
describe('producedFileMentions resolver', () => {
  ...
  it('resolves exact paths and unique basenames; ambiguity and unknowns stay unresolved', () => {
    ...
    expect(parentDirectory('a/b/c.txt')).toBe('a/b')
