...
export const DESKTOP_BOOT_HEALTH_EVENT = 'dsh:desktop-boot-health'

function reportDesktopBootHealth(detail: { ok: boolean; entries: string[]; error?: string }): void {
  globalThis.dispatchEvent(new CustomEvent(DESKTOP_BOOT_HEALTH_EVENT, { detail }))
}
...
export class AppWebEntry {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      reportDesktopBootHealth({
        ok: true,
        entries: this.manifest.plugins.map(row => row.id),
      })
    } catch (reason) {
