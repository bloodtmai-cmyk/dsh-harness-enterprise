...
async function harness(
  ...
  options: { allowUserDefault?: boolean; settings?: string } = {},
  ...
  await ctx.plugin(AgentPresets, {
    ...
    ...(options.allowUserDefault === undefined ? {} : { allowUserDefault: options.allowUserDefault }),
  })
...
describe('the default preset as a user setting', () => {
  ...
  it('pins the composition default when user overrides are disabled', async () => {
    const { ctx } = await harness([], {
