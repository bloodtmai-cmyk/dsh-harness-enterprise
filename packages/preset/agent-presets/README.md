...
| `allowUserDefault` | `true` | Register the settings override for `default`; `false` pins the deployment default |
...
`allowUserDefault: false` does not register this namespace. An existing `agent-presets.default` value in the settings document is then ignored, and `defaultId` always resolves to the deployment's `config.default`. Managed products use this together with `includeUserRoot: false` when preset choice and preset contents are deployment policy rather than end-user configuration.
