...
| `allowUserDefault` | `true` | 是否注册覆盖 `default` 的用户设置；设为 `false` 时固定使用部署默认值 |
...
`allowUserDefault: false` 不会注册这个命名空间。此时 settings 文档中已有的 `agent-presets.default` 会被忽略，`defaultId` 始终解析为部署方的 `config.default`。当 preset 选择及其内容属于部署策略而非终端用户配置时，托管产品会将它与 `includeUserRoot: false` 配合使用。
