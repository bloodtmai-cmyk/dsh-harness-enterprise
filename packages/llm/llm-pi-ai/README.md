...
The package root exposes the Cordis plugin contract, `PiAiAdapter`, `resolveProfiles()`, `discoverModels()`, and `supportedProtocols()` so deployment-owned adapters can reuse the validated route and discovery machinery; catalog materialization, provider construction, replay conversion, and stream conversion remain package-internal.
