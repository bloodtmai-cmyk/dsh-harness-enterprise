...
function flattenText(message: Message): string {
  return message.content
    .flatMap(block => block.type === 'text'
      ? [block.text]
      : block.type === 'document' ? [documentText(block)] : [])
    .join('')
}

/** Provider-facing representation of one admitted Office document. */
function documentText(block: Extract<ContentBlock, { type: 'document' }>): string {
  const suffix = block.truncated ? '\n[内容因长度限制已截断]' : ''
  return `\n<document name="${block.attachment.name}">\n${block.text}${suffix}\n</document>\n`
}
...
async function userContent(
  ...
  for (const block of blocks) {
    switch (block.type) {
      ...
      case 'document':
        content.push({ type: 'text', text: documentText(block) })
        break
      case 'tool-result':
        {
