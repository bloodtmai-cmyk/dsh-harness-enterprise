...
/** A durable Office document plus the bounded text extracted during admission. */
export interface DocumentBlock {
  type: 'document'
  /** Immutable original Office file metadata. */
  attachment: DocumentAttachmentRef
  /** Plain text extracted from the verified OOXML package for model input. */
  text: string
  /** Whether extraction reached the deployment character limit. */
  truncated: boolean
}

/** JSON-only data accepted by durable chart blocks. */
export type ChartValue = null | boolean | number | string | ChartValue[] | { [key: string]: ChartValue }

/**
 * A presentation-only chart returned by a Tool. Providers ignore this block
 * while retaining the neighboring text block as model-visible evidence.
 */
export interface ChartBlock {
  type: 'chart'
  /** GPT-Vis chart type after MCP boundary validation. */
  chartType: string
  /** Bounded GPT-Vis JSON configuration, including its `type` field. */
  payload: { [key: string]: ChartValue }
}
