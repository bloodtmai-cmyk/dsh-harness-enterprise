...
The same package can separately mount one trusted `managedInstructionFile` plus its read-only `managedInstructionHashFile` as an order `-90` system-prompt section. Both files are validated at startup, then reread before every model request so an atomically delivered enterprise version hot-loads without restarting Harness. If a refresh observes an incomplete or invalid file/hash pair, the reader keeps the last validated policy instead of dropping protection. The content is framed as mandatory enterprise policy and inserted through a prompt variable, so literal `{{...}}` text is not interpreted. A managed deployment can set `maxBytes: 0` to disable every user-global, project, CLAUDE, and local-overlay instruction while retaining this enterprise section; Harness Enterprise Desktop uses exactly that policy.
...
`managedInstructionFile` is independent of workspace discovery. When nonempty, `managedInstructionHashFile` must name a non-symlink, read-only sidecar containing the expected lowercase SHA-256. Startup validates both files and fails rather than silently dropping enterprise policy when either file is missing, writable, malformed, integrity-mismatched, invalid UTF-8, empty, or oversized. The prompt variable rereads and validates the pair before every model request, adopts a complete new generation immediately, and retains the last validated prompt when it observes a partial or invalid refresh. The policy remains a system section immediately after the fixed Harness identity and before the deployment persona.
