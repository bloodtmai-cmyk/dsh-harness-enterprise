...
同一个包还可以把可信 `managedInstructionFile` 及其只读 `managedInstructionHashFile` 挂载为顺序 `-90` 的 system prompt 段。两个文件在启动时完成校验，并在每次模型请求前重新读取，因此原子下发的新企业指令版本无需重启 Harness 即可热加载。若刷新期间读到不完整或无效的文件／哈希组合，loader 会继续使用上一份已校验策略，而不是丢失保护。内容会被框定为强制企业策略，并通过 prompt 变量插入，因此字面量 `{{...}}` 不会被解释。受管部署可设置 `maxBytes: 0`，在保留企业指令段的同时关闭所有用户全局、项目、CLAUDE 和本地 overlay 指令；Harness Enterprise Desktop 正是采用此策略。
...
`managedInstructionFile` 与工作区发现相互独立。配置非空时，`managedInstructionHashFile` 必须指向一份非 symlink、只读且包含预期小写 SHA-256 的 sidecar。启动时会校验两个文件；任一文件缺失、可写、格式错误、完整性不匹配，或指令文件不是有效 UTF-8、为空、超限时，都会失败，而不是静默丢弃企业策略。prompt 变量会在每次模型请求前重新读取并校验这一对文件，完整的新 generation 会立即生效；遇到部分更新或无效刷新时则保留上一份已校验 prompt。该策略仍位于固定 Harness 身份之后、部署 persona 之前的 system 段。
