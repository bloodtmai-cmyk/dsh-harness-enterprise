...
import { z } from 'zod'
import {
  attachmentRefSchema, contentBlockSchema, sessionAttachmentValueSchema,
...
describe('sessions domain schemas', () => {
  ...
  it('accepts authenticated image and Office attachment values without mixing their metadata', () => {
    const image = {
      attachmentId: 'image-1', mediaType: 'image/png', bytes: 2, width: 1, height: 1,
    }
    const document = {
    ...
    expect(attachmentRefSchema.parse(image)).toMatchObject({ attachmentId: 'image-1' })
    expect(sessionAttachmentValueSchema.parse({ attachment: document, data: 'AQID' }))
      .toMatchObject({ attachment: { attachmentId: 'document-1', name: 'report.xlsx' } })
    expect(() => attachmentRefSchema.parse({ ...document, mediaType: 'image/png' })).toThrow()
  })
})
