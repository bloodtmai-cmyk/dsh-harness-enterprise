...
describe('Web session model selection', () => {
  ...
  it('authorizes image and Office bytes only when the session event stream references the id', async () => {
    const { ctx, agent, sessionId } = await harness()
    const ref = {
      attachmentId: 'att-authorized', mediaType: 'image/png' as const, bytes: 2, width: 1, height: 1,
    }
    const documentRef = {
      attachmentId: 'doc-authorized',
    ...
    const readDocument = vi.fn(() => Promise.resolve({
      ref: documentRef,
      data: Uint8Array.of(3, 4, 5),
    }))
    ctx.provide('attachments', { readImage, readDocument } as never)
    const api = createApiProxy(ctx, {
      defaultModelSelection: () => ({ provider: 'deepseek-official', model: 'deepseek-chat' }),
      cwd: '/tmp',
    })
    agent.session.append('agent/inbox/spliced', {
      ...
      start: 0,
      inserted: [{
        id: 'queued-attachments', role: 'user', source: { kind: 'user' },
        content: [
          { type: 'image', attachment: ref },
          { type: 'document', attachment: documentRef },
        ],
      }],
    } as never)
    ...
    const allowedDocument = await api.sessions.attachment(request({
      sessionId, attachmentId: 'doc-authorized' as never,
    }))
    expect(allowedDocument.result).toMatchObject({
      ok: true,
      value: { attachment: documentRef, data: 'AwQF' },
    })
