...
describe('settings domain', () => {
  ...
  it('hides and rejects the settings document when the deployment disables raw editing', async () => {
    const ctx = await harness({ settings: {
      documentPath: '/tmp/described-settings.yaml',
      preparedPath: '/tmp/custom-settings.yaml',
    } })
    ...
    expect(error.message).toContain('disabled by this deployment')
  ...
  it('lets managed deployments restrict settings reads, writes, and model discovery', async () => {
    const ctx = await harness()
    ctx.settings.register(settingsNamespace('ui-theme'), z.object({
      preference: z.union(['light', 'dark', 'system']).default('system'),
    }))
    ...
    const api = createApiProxy(ctx, {
      ...DEFAULTS,
      settingsNamespaceAllowlist: ['ui-theme'],
    })

    expect(expectOk(await api.settings.describe(request({}))).namespaces.map(view => view.ns))
      .toEqual(['ui-theme'])
    const deniedWrite = expectErr(await api.settings.mutate(request({
      ns: 'permission',
      ops: [{ op: 'set', path: ['defaultPreset'], value: 'workspace-write' }],
    })))
    expect(deniedWrite.code).toBe('settings-rejected')
    expect(deniedWrite.message).toContain('not exposed by this deployment')

    const deniedDiscovery = expectErr(await api.llm.discoverModels(request({
      settingsNs: 'llm-deepseek',
      baseURL: 'https://models.example.test/v1',
    })))
    expect(deniedDiscovery.code).toBe('model-discovery-failed')
    expect(deniedDiscovery.message).toContain('disabled by this deployment')
  })
