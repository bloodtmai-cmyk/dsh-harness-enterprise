...
`settingsDocumentOpen: false` 会保留已暴露偏好 namespace 的类型化控件写入能力，但强制返回 `hasDocument: false`，并在提供方准备路径前拒绝 `settings.openDocument`。托管部署用它关闭原始文件编辑，同时保留普通偏好设置。
