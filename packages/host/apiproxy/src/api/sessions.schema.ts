...
/** Modern Office Open XML media types accepted by the browser wire. */
...
export const promptContentPartSchema = z.discriminatedUnion('type', [
  ...
  z.object({ type: z.literal('document'), mediaType: documentMediaTypeSchema, data: z.string(), name: z.string().min(1).max(255) }),
])
...
/** Durable Office reference returned from the authenticated session lookup. */
...
/** Durable image or Office reference returned from the authenticated session lookup. */
export const attachmentRefSchema = z.union([
  imageAttachmentRefSchema,
  documentAttachmentRefSchema,
]) as unknown as z.ZodType<AttachmentRef>
