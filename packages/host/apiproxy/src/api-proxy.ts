...
import type { AttachmentRef } from '@deepseek-ai/dsh-attachment'
...
import {
  documentLimitsProjectionSchema,
...
function decodeBase64(data: string, kind: 'image' | 'document'): Uint8Array {
  ...
  if (data.length === 0 || decoded.toString('base64') !== data) {
    throw new AttachmentError(
      `${kind === 'image' ? 'Image' : 'Document'} upload is not canonical base64.`,
...
async function durablePromptContent(ctx: Context, content: readonly PromptContentPart[]): Promise<ContentBlock[]> {
  ...
  const images = prepared.filter((item): item is {
    part: Extract<PromptContentPart, { type: 'image' }>
    data: Uint8Array
  } =>
    'data' in item && item.part.type === 'image')
  const documents = prepared.filter((item): item is {
    part: Extract<PromptContentPart, { type: 'document' }>
    data: Uint8Array
  } =>
    'data' in item && item.part.type === 'document')
  ...
  const savedDocuments = documents.length === 0 ? [] : await ctx.attachments.saveDocuments(documents.map(document => ({
    data: document.data,
    mediaType: document.part.mediaType,
    name: document.part.name,
  })))
  ...
  for (const item of prepared) {
    ...
    if (item.part.type === 'image') {
      ...
      /* v8 ignore next -- each prepared image supplied exactly one ordered reference. */
      if (attachment === undefined) throw new Error('image attachment batch result did not preserve input cardinality')
      blocks.push({ type: 'image', attachment })
      continue
    }
    const document = savedDocuments[documentIndex++]
    /* v8 ignore next -- each prepared document supplied exactly one ordered result. */
    if (document === undefined) throw new Error('document attachment batch result did not preserve input cardinality')
    blocks.push({ type: 'document', attachment: document.ref, text: document.text, truncated: document.truncated })
  }
  return blocks
}

/** Search durable content for an attachment reference, including nested tool results. */
function attachmentBlockIn(content: unknown, match: (ref: AttachmentRef) => boolean): AttachmentRef | undefined {
  ...
  for (const value of content) {
    ...
    if ((block.type === 'image' || block.type === 'document')
      ...
      const ref = block.attachment as AttachmentRef
      if (match(ref)) return ref
    }
    if (block.type === 'tool-result') {
      const nested = attachmentBlockIn(block.content, match)
...
function attachmentInEvent(event: SessionEvent, match: (ref: AttachmentRef) => boolean): AttachmentRef | undefined {
  ...
  const direct = attachmentBlockIn(data.content, match)
  if (direct !== undefined) return direct
  if (data.message !== undefined) {
    const wrapped = attachmentBlockIn(data.message.content, match)
    if (wrapped !== undefined) return wrapped
  }
  if (data.inserted !== undefined) {
    for (const message of data.inserted) {
      const inserted = attachmentBlockIn(message.content, match)
      if (inserted !== undefined) return inserted
    }
  }
  if (event.type === 'assistant/chunk' && data.chunk?.type === 'block-end') {
    return attachmentBlockIn([data.chunk.block], match)
  }
  return undefined
}
...
function referencedAttachment(events: readonly SessionEvent[], attachmentId: string): AttachmentRef | undefined {
  for (const event of events) {
    const found = attachmentInEvent(event, ref => String(ref.attachmentId) === attachmentId)
...
export interface ApiProxyDefaults {
  ...
  /** Whether clients may prepare and open the provider-owned settings document. */
  settingsDocumentOpen?: boolean
  /**
   * Optional deployment-owned allowlist for settings namespaces exposed to
   * clients. Undefined keeps the upstream plugin-owned settings behavior;
   * an empty list exposes none.
   */
  settingsNamespaceAllowlist?: readonly string[]
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  const settingsNamespaceAllowlist = defaults.settingsNamespaceAllowlist === undefined
    ? undefined
    : new Set(defaults.settingsNamespaceAllowlist)
  const settingsNamespaceAllowed = (ns: string): boolean =>
    settingsNamespaceAllowlist === undefined || settingsNamespaceAllowlist.has(ns)
  ...
  async function settingsWrite(
    ...
    if (!settingsNamespaceAllowed(ns)) {
      return rejected(new Error(`settings namespace "${ns}" is not exposed by this deployment`))
    }
    try {
  ...
  return {
    sessions: {
      ...
      async attachment(request) {
        ...
        const ref = referencedAttachment(state.events, String(attachmentId))
        if (ref === undefined) {
          return err(request, {
            code: 'attachment-error',
        ...
        try {
          const stored = 'width' in ref
            ? await ctx.attachments.readImage(ref)
            : await ctx.attachments.readDocument(ref)
          return ok(request, {
    ...
    settings: {
      describe(request) {
        ...
        return Promise.resolve(ok(request, {
          ...
          hasDocument: defaults.settingsDocumentOpen !== false && settings.documentPath !== undefined,
          namespaces: settings.describe({ redactSecrets: true })
            .filter(descriptor => settingsNamespaceAllowed(String(descriptor.ns)))
            .map(namespaceView),
        }))
      },
      async openDocument(request, signal) {
        const settings = ctx.get('settings')
        if (settings === undefined) return err(request, settingsAbsent())
        if (defaults.settingsDocumentOpen === false) {
          return err(request, {
            code: 'internal',
            message: 'settings document opening is disabled by this deployment',
            details: {},
          })
        }
        if (isAborted(signal)) {
    ...
    llm: {
      ...
      async discoverModels(request, signal) {
        ...
        if (!settingsNamespaceAllowed(settingsNs)) {
          return err(request, {
            code: 'model-discovery-failed',
            message: `model discovery for settings namespace "${settingsNs}" is disabled by this deployment`,
