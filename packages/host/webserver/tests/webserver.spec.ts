...
const ACCESS_TOKEN_ENV = 'DSH_TEST_WEB_ACCESS_TOKEN'
...
afterEach(async () => {
  ...
  root = undefined
  delete process.env.DSH_TEST_WEB_ACCESS_TOKEN
})

/** Write a cordis.yml with one webserver row, then boot it through the real Loader. */
async function loadComposition(port = 0, accessTokenEnv?: string): Promise<Context> {
...
async function upgrade(port: number, path: string, token?: string): Promise<ReturnType<typeof connect>> {
...
describe('real Loader composition', () => {
  ...
  it('rejects browser HTTP and upgrade requests without the configured bearer token', { timeout: 60_000 }, async () => {
    process.env[ACCESS_TOKEN_ENV] = ACCESS_TOKEN
    const loaded = await loadComposition(0, ACCESS_TOKEN_ENV)
    const server = loaded.webServer
    const port = server.port
    server.register({
      kind: 'exact',
      path: '/probe',
      handler: (_req, res) => { res.writeHead(200); res.end('PROTECTED') },
    })
    server.registerUpgrade({
      path: '/events',
      handler: (_req, socket) => {
    ...
    expect(await request(port, '/probe', {
      headers: { Authorization: `Bearer ${'x'.repeat(ACCESS_TOKEN.length)}` },
    })).toMatchObject({ status: 401, body: 'unauthorized' })
    expect(await request(port, '/probe', {
      headers: { Authorization: `Bearer ${ACCESS_TOKEN}` },
    })).toMatchObject({ status: 200, body: 'PROTECTED' })

    const denied = connect(port, '127.0.0.1')
    await once(denied, 'connect')
    const denial = once(denied, 'data')
    denied.write([
      'GET /events HTTP/1.1',
      `Host: 127.0.0.1:${String(port)}`,
      'Connection: Upgrade',
    ...
    const [deniedResponse] = await denial as [Buffer]
    expect(String(deniedResponse)).toContain('401 Unauthorized')
    denied.destroy()

    const accepted = await upgrade(port, '/events', ACCESS_TOKEN)
    accepted.destroy()
  })

  it('fails closed when a configured access-token environment variable is missing', { timeout: 60_000 }, async () => {
    await expect(loadComposition(0, ACCESS_TOKEN_ENV))
      .rejects.toThrow(/must contain a 32-256 character access token/)
  })
