...
export class WebServer extends Service {
  ...
  private readonly accessToken: Buffer | undefined

  constructor(ctx: Context, private config: Config) {
    super(ctx, 'webServer')
    if (config.accessTokenEnv !== undefined) {
      const token = process.env[config.accessTokenEnv]
      if (token === undefined || !/^[A-Za-z0-9_-]{32,256}$/.test(token)) {
        throw new Error(`webserver: ${config.accessTokenEnv} must contain a 32-256 character access token`)
      }
      this.accessToken = Buffer.from(token)
    }
  }
  ...
  private isAuthorized(req: IncomingMessage): boolean {
    ...
    return supplied.length === this.accessToken.length && timingSafeEqual(supplied, this.accessToken)
  }
