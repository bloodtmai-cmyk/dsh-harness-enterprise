...
| `bearerTokenBroker.url` | http | 否 | 仅允许 `http://127.0.0.1:<port>/token` 的受管 Bearer Token Broker |
| `bearerTokenBroker.secret` | http | 否 | 调用本地 Broker 的每进程随机秘密，至少 32 个字符 |
| `toolCallTimeoutMs` | 两者 | 否 | 每次 `callTool` 调用的超时（默认 60000） |
| `failOnStartupError` | 两者 | 否 | 初始连接或工具同步失败时拒绝插件激活（默认 `false`） |
| `toolGuidance` | 两者 | 否 | 追加到每个模型可见工具描述后的部署侧策略 |
| `includeSessionMeta` | 两者 | 否 | 只把调用方 Harness Agent 的不透明 ID 作为 `_meta.session_id` 发送；仅受信 Gateway 可以启用（默认 `false`） |
| `toolRefreshIntervalMs` | 两者 | 否 | 服务端无法发送通知时，轮询 `tools/list` 以同步授权变化；0 表示禁用，非 0 时最小为 1000（默认 0） |
| `jsonErrorMessages` | 两者 | 否 | JSON 结果信封 `code` 到安全错误文案的映射；命中后提升为失败的工具调用 |
...
- 监听 `notifications/tools/list_changed` → 重新同步。受管 Gateway 无法按用户授权变化发送通知时，也可以设置 `toolRefreshIntervalMs` 做有界的定期同步。获取阶段失败时保留上一世代的注册，注册冲突则会回滚本次尝试的世代，并且不保留该服务器的任何工具。
- 工具执行：`client.callTool({ name: rawName, arguments }, { signal })`，支持超时 + 中止；公开名称绝不会发给服务器。
- 配置本地 Bearer Token Broker 时，每个 HTTP 请求都从严格限制在 `127.0.0.1` 的 Broker 取得 Token 并覆盖 `Authorization`；服务端返回 401 后强制刷新并最多重试一次，Broker 秘密和 Token 都不会进入模型参数。
- `jsonErrorMessages` 是一个显式开启的兼容边界，用于企业适配器把 JSON 业务错误信封返回成 MCP 成功结果的场景。只有配置过的错误码会被提升为失败，其余 JSON 仍是普通成功输出。
