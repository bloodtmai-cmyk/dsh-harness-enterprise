...
export function startConnection(ctx: Context, config: Config, policy: ResolvedReconnectPolicy): ConnectionHandle {
  ...
  const toolRefreshIntervalMs = config.toolRefreshIntervalMs ?? 0
  if (!Number.isInteger(toolRefreshIntervalMs) || toolRefreshIntervalMs < 0
    || toolRefreshIntervalMs > MAX_TIMER_DELAY_MS
    || (toolRefreshIntervalMs > 0 && toolRefreshIntervalMs < 1_000)) {
    throw new Error(`${label}: toolRefreshIntervalMs must be zero or an integer between 1000 and ${MAX_TIMER_DELAY_MS}`)
  }
  const opts: ToolBridgeOptions = {
    ...(config.jsonErrorMessages === undefined ? {} : { jsonErrorMessages: config.jsonErrorMessages }),
    registrationFailure: 'contain',
    serverName: config.serverName,
    toolCallTimeoutMs: config.toolCallTimeoutMs,
    ...(config.toolGuidance === undefined ? {} : { toolGuidance: config.toolGuidance }),
    ...(config.includeSessionMeta === undefined ? {} : { includeSessionMeta: config.includeSessionMeta }),
  }
  ...
  function stopToolRefresh(): void {
    if (toolRefreshTimer !== undefined) clearInterval(toolRefreshTimer)
    toolRefreshTimer = undefined
    toolRefreshRunning = false
  }

  function startToolRefresh(generation: Client): void {
    stopToolRefresh()
    if (toolRefreshIntervalMs === 0) return
    toolRefreshTimer = setInterval(() => {
      if (!isCurrent(generation) || toolRefreshRunning) return
      toolRefreshRunning = true
      void enqueueSync(generation)
        .catch((error: unknown) => {
          if (!disposed) ctx.logger.error(`${label}: periodic tool re-sync failed: ${String(error)}`)
        })
        .finally(() => { toolRefreshRunning = false })
