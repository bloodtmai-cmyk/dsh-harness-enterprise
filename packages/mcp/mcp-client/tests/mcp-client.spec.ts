...
describe('tool execution', () => {
  ...
  it('sends only an opaque Harness session id when enterprise session metadata is enabled', async () => {
    const client = createMockClient([{ name: 'scoped', inputSchema: { type: 'object' } }])

    await syncTools(client as never, ctx, { ...defaultOpts, includeSessionMeta: true }, new Map())
    await ctx.tools.execute({
      signal: testToolSignal,
      callId: CallId('session-meta'),
      name: 'mcp__srv__scoped',
      arguments: {},
      agent: { ...agentOn(), id: 'opaque-session-1234' } as never,
    })

    expect(client.callTool).toHaveBeenCalledWith({
      name: 'scoped',
      arguments: {},
      _meta: { session_id: 'opaque-session-1234' },
    }, undefined, expect.any(Object))
  })

  it('preserves a bounded MCP chart envelope and projects a GPT-Vis block', async () => {
    const meta = {
      type: 'chart',
      chart_type: 'column',
      payload: {
        type: 'column',
        title: '月度销售额',
        data: [{ category: '1月', value: 120 }, { category: '2月', value: 180 }],
      },
    } satisfies JsonValue
    const client = createMockClient(
      [{ name: 'generate_column_chart', inputSchema: { type: 'object' } }],
      { content: [{ type: 'text', text: '柱状图已生成。' }], _meta: meta },
    )

    await syncTools(client as never, ctx, defaultOpts, new Map())
    const result = await ctx.tools.execute({
      signal: testToolSignal,
      callId: CallId('chart'), name: 'mcp__srv__generate_column_chart', arguments: {},
    })

    expect(result.content).toEqual([
      { type: 'text', text: '柱状图已生成。' },
      { type: 'chart', chartType: 'column', payload: meta.payload },
    ])
    if (result.isError) throw new Error('expected MCP chart success')
    expect(result.value).toEqual({
      content: [{ type: 'text', text: '柱状图已生成。' }],
      meta,
    })
  })

  it('ignores unsupported chart metadata without losing the text result', async () => {
    const client = createMockClient(
      [{ name: 'chart', inputSchema: { type: 'object' } }],
      {
        content: [{ type: 'text', text: 'kept' }],
        _meta: { type: 'chart', chart_type: 'arbitrary-script', payload: { type: 'arbitrary-script' } },
      },
    )

    await syncTools(client as never, ctx, defaultOpts, new Map())
    const result = await ctx.tools.execute({
      signal: testToolSignal, callId: CallId('bad-chart'), name: 'mcp__srv__chart', arguments: {},
    })
    expect(result.content).toEqual([{ type: 'text', text: 'kept' }])
    if (result.isError) throw new Error('unsupported chart metadata must not fail text output')
    expect(result.value).toEqual({ content: [{ type: 'text', text: 'kept' }] })
  })
  ...
  it('promotes explicitly configured JSON envelope codes to failed tool results', async () => {
    const client = createMockClient(
      [{ name: 'inventory', inputSchema: { type: 'object' } }],
      { content: [{ type: 'text', text: '{"code":4003,"message":"Authentication failed","data":null}' }] },
    )

    await syncTools(client as never, ctx, {
      ...defaultOpts,
      jsonErrorMessages: { 4003: 'Downstream authentication is misconfigured; do not request user credentials.' },
    }, new Map())
    const result = await ctx.tools.execute({
      signal: testToolSignal,
      callId: CallId('business-error'), name: 'mcp__srv__inventory', arguments: {},
    })

    expect(result.isError).toBe(true)
    expect(result.content[0]).toEqual({
      type: 'text',
      text: 'Error: Downstream authentication is misconfigured; do not request user credentials.',
    })
  })

  it('keeps unconfigured JSON envelopes as successful tool output', async () => {
    const envelope = '{"code":0,"data":{"stock":12}}'
    const client = createMockClient(
      [{ name: 'inventory', inputSchema: { type: 'object' } }],
    ...
    await syncTools(client as never, ctx, {
      ...defaultOpts,
      jsonErrorMessages: { 4003: 'Downstream authentication is misconfigured.' },
    }, new Map())
    const result = await ctx.tools.execute({
      signal: testToolSignal,
      callId: CallId('business-success'), name: 'mcp__srv__inventory', arguments: {},
    })

    expect(result.isError).toBe(false)
    expect(result.content[0]).toEqual({ type: 'text', text: envelope })
  })
...
describe('tool execution edge cases', () => {
  ...
  it('appends deployment guidance to each model-facing tool description', async () => {
    const client = createMockClient([
      { name: 'managed', description: 'Remote description', inputSchema: { type: 'object' } },
    ])

    await syncTools(client as never, ctx, {
      ...defaultOpts,
      toolGuidance: 'Identity is injected by the gateway; never ask the user for credentials.',
    }, new Map())

    expect(ctx.tools.get('mcp__srv__managed')?.description).toBe(
      'Remote description\n\nIdentity is injected by the gateway; never ask the user for credentials.',
    )
  })
