...
describe('reconnect supervisor', () => {
  ...
  it('periodically reconciles tools for managed gateways without list-changed notifications', async () => {
    vi.useFakeTimers()
    const handle = startConnection(
      ctx,
      { ...stdioConfig(), toolRefreshIntervalMs: 1_000 },
    ...
    try {
      ...
      mockListTools.mockResolvedValue(listing('newly_authorized'))
      await vi.advanceTimersByTimeAsync(1_000)

      expect(mockListTools).toHaveBeenCalledTimes(2)
      expect(ctx.tools.get('mcp__srv__remote')).toBeUndefined()
      expect(ctx.tools.get('mcp__srv__newly_authorized')).toBeDefined()
    } finally {
  ...
  it('rejects a nonzero tool refresh interval below one second', () => {
    expect(() => startConnection(
      ctx,
      { ...stdioConfig(), toolRefreshIntervalMs: 999 },
      resolveReconnectPolicy(undefined, 'reconnect'),
    )).toThrow(/toolRefreshIntervalMs/)
  })
