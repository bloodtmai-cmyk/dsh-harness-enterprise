...
describe('mcp-client plugin module exports', () => {
  ...
  it('Config schema accepts Streamable HTTP without an optional token broker', () => {
    const resolved = ConfigSchema({
    ...
    expect(resolved.bearerTokenBroker).toBeUndefined()
  })

  it('Config schema accepts deployment tool guidance and JSON error promotion', () => {
    const resolved = ConfigSchema({
      transport: 'streamable-http',
      serverName: 'workbuddy',
      url: 'https://workbuddy.example/mcp',
      toolGuidance: 'Identity is gateway-owned.',
      jsonErrorMessages: { 4003: 'Downstream authentication is misconfigured.' },
    } as never)

    expect(resolved.toolGuidance).toBe('Identity is gateway-owned.')
    expect(resolved.jsonErrorMessages).toEqual({ 4003: 'Downstream authentication is misconfigured.' })
  })
