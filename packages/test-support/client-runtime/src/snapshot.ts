 ...
 * - Inline base64 image URLs in style attributes collapse to fingerprints so
 *   a small structural snapshot does not duplicate whole bitmap assets.
 */
...
const INLINE_IMAGE = /data:(image\/[a-z0-9.+-]+);base64,([^"')\s;]+)/gi
...
function normalizeInlineImages(value: string): string {
  return value.replace(INLINE_IMAGE, (_match, mediaType: string, payload: string) =>
    `data:${mediaType};fingerprint=${fingerprint(payload)}`)
}
...
function needsNormalization(root: Element): boolean {
  ...
  const inlineImage = [root, ...root.querySelectorAll('[style]')].some(el =>
    INLINE_IMAGE.test(el.getAttribute('style') ?? ''))
  INLINE_IMAGE.lastIndex = 0
  return scoped || inlineImage || svgsOf(root).some(svg => svg.childNodes.length > 0)
}
...
export const domSnapshotSerializer: SnapshotSerializer = {
  ...
  serialize(value, config, indentation, depth, refs, printer): string {
    ...
    for (const el of [clone, ...clone.querySelectorAll('[style]')]) {
      const raw = el.getAttribute('style')
      if (raw !== null) el.setAttribute('style', normalizeInlineImages(raw))
    }
